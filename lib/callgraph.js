...
Callgraph.prototype.generateImageByDot = function (script, type, callback) {
  var inStream = new streamers.BufferReadStream(script);
  var outStream = new streamers.BufferWriteStream();
  ...
  proc.stdin.on('close', function () {
    callback(null, new streamers.BufferReadStream(outStream.getBuffer()));
  });
